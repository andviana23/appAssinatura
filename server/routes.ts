...
import { eq, like, sql, and, gte, lte, or } from 'drizzle-orm';
...
export async function registerRoutes(app: Express): Promise<Express> {
  ...
  app.post('/api/clientes/importar', requireAdmin, upload.single('planilha'), async (req: any, res: Response) => {
    try {
      ...
      // Se arquivo foi enviado, processar planilha
      if (req.file) {
        try {
          ...
          clientesParaImportar = dados.map((linha: any) => ({
            nome: linha.nome || linha.Nome || linha.NOME || '',
            cpf: linha.cpf || linha.CPF || linha.Cpf || '',
            email: linha.email || linha.Email || linha.EMAIL || '',
            telefone: linha.telefone || linha.Telefone || linha.TELEFONE || '',
            proximoVencimento: linha.proximoVencimento || linha['Próximo Vencimento'] || linha.vencimento || '',
            plano: linha.plano || linha.Plano || linha.PLANO || 'Barba Mensal'
          }));
        } catch (error) {
          console.error('Erro ao processar planilha:', error);
          return res.status(400).json({ 
            message: 'Erro ao processar planilha. Verifique o formato do arquivo.',
            error: error.message 
          });
        }
      } 
      // Se dados manuais foram enviados
      else if (req.body.clientes) {
        clientesParaImportar = req.body.clientes;
      } else {
        return res.status(400).json({ 
          message: 'Nenhum arquivo ou dados foram fornecidos' 
        });
      }
      ...
      for (const clienteData of clientesParaImportar) {
        try {
          // Verificar se cliente já existe por CPF, email ou telefone
          const clienteExistente = await db.select()
            .from(schema.clientes)
            .where(or(
              eq(schema.clientes.cpf, clienteData.cpf),
              eq(schema.clientes.email, clienteData.email),
              eq(schema.clientes.telefone, clienteData.telefone)
            ))
            .limit(1);
          ...
          let status = 'ativo';
          if (clienteData.proximoVencimento) {
            const vencimento = new Date(clienteData.proximoVencimento);
            const hoje = new Date();
            if (vencimento < hoje) {
              status = 'inativo';
            }
          ...
          await db.insert(schema.clientes).values({
            ...
            valorAssinatura: 0, // Será definido ao gerar cobrança
            origem: 'planilha'
          });
      ...
      res.json({
        success: true,
        message: `Importação concluída: ${resultados.cadastrados} cadastrados, ${resultados.pulados} pulados`,
        resultados
      });
  ...
  // Endpoint para gerar cobrança para cliente inativo
  app.post('/api/clientes/:id/gerar-cobranca', requireAnyRole, async (req: any, res: Response) => {
    try {
      const clienteId = parseInt(req.params.id);
      const { planoId, cpf } = req.body;

      if (!cpf) {
        return res.status(400).json({ message: 'CPF é obrigatório para gerar cobrança' });
      }
      ...
      if (!asaasApiKey) {
        return res.status(500).json({ message: 'Chave da API Asaas não configurada' });
      }
      ...
      const cobrancaResult = await response.json();

      if (!response.ok) {
        throw new Error(cobrancaResult.errors?.[0]?.description || 'Erro ao criar cobrança');
      }

      // Atualizar cliente com informações da cobrança
      await db.update(schema.clientes)
        .set({
          asaasCustomerId: cpf,
  ...
  // Endpoint para marcar pagamento como pago (externo)
  app.post('/api/clientes/:id/marcar-pago', requireAnyRole, async (req: any, res: Response) => {
    try {
      const clienteId = parseInt(req.params.id);

      // Atualizar status do cliente para ativo e adicionar 30 dias
